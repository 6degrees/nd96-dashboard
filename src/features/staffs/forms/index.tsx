...
import {RoleDropdownFormItem} from "@/features/staffs";
...
|
*/
export default function StaffForm({ onSubmit, data, isEdit = false }: FormProps) {
    /*
    |--------------------------------------------------------------------------
    | Translation
    |--------------------------------------------------------------------------
    ...
    */
    return (
        <Form form={form} layout="vertical" onFinish={onSubmit} initialValues={{ 'company': params?.id }}
            ...
            {/* Basic Information */}
            <FormCard title={'staff.create'}>
                <Row gutter={[24, 24]}>
                    <Col xs={24} md={24}>
                        <EmailFormItem name="email" label="staff.inputs.email" placeholder="staff.inputs.emailPlaceholder" rules={[requiredRule(t('validation.requiredField'))]} />
                    </Col>
                </Row>

                <Col xs={24} md={24}>
                    <RoleDropdownFormItem name="role" label="staff.inputs.role" placeholder="staff.inputs.rolePlaceholder" rules={[requiredRule(t('validation.requiredField')),]} />
                </Col>
            </FormCard>
            ...
            <div className="flex">
                <div className="ms-auto">
                    <SubmitButtonFormItem label={isEdit ? 'UPDATE' : 'staff.create'} loading={actionLoading} background="linear-gradient(to right, #020618, #0f172a, #1e293b)" />
                </div>
            </div>
        </Form>
    )
