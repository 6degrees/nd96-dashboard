    ...
    return (
        <>
            <Col xs={24} sm={12} md={8}>
                <label>{t('customer.inputs.name')}</label>
                <FilterTextFormItem value={filters.name} onChange={(value) => setFilters({ ...filters, name: value })}/>
            </Col>

            <Col xs={24} sm={12} md={8}>
                <label>{t('customer.inputs.email')}</label>
                <FilterTextFormItem value={filters.email} onChange={(value) => setFilters({ ...filters, email: value })}/>
            </Col>

            <Col xs={24} sm={12} md={8}>
                <label>{t('customer.inputs.phone')}</label>
                <FilterTextFormItem value={filters.mobile} onChange={(value) => setFilters({ ...filters, mobile: value })}/>
            </Col>
